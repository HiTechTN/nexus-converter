...
if [[ $? -eq 0 ]]; then
  ...
  warn "  → Essayez: pnpm install --no-frozen-lockfile"
...
pnpm --filter @workspace/api-zod run build 2>&1 | tail -3 && \
  log "api-zod: build réussi" || \
  warn "api-zod: échec du build"

info "Build de @workspace/api-client-react..."
pnpm --filter @workspace/api-client-react run build 2>&1 | tail -3 && \
  log "api-client-react: build réussi" || \
  warn "api-client-react: échec du build"
...
echo -e "${BOLD}╚══════════════════════════════════════════════════════════╝${RESET}"
echo ""
echo -e "  ${CYAN}▶${RESET} Pour lancer l'application :"
echo ""
...
echo ""
echo -e "  ${CYAN}▶${RESET} Docker (alternative) :"
